...
We compared all the coefficients in this study with MIC [@pmid:22174245], a popular nonlinear method that can detect complex relationships in data, although it is computationally intensive [@doi:10.1098/rsos.201424].
...
This took 4 days and 19 hours to complete (compared with 9 hours for CCC).
Then, we analyzed the distribution of coefficients (the same as in the main text), shown in Figure @fig:dist_coefs_mic.
We verified that CCC and MIC behaved similarly in this dataset, with essentially the same distribution but only shifted.
Figure @fig:dist_coefs_mic c indicates that these two coefficients have almost a linear relationship, and both compare very similarly with Pearson and Spearman.
...
We also compared CCC with other correlation coefficients in terms of computational complexity.
CCC and MIC might identify similar gene pairs in gene expression data (see [here](#sec:mic)).
However, the use of MIC in large datasets is limited due to its long computation time, despite some improvements [@doi:10.1093/bioinformatics/bts707; @doi:10.1371/journal.pone.0157567; @doi:10.4137/EBO.S13121; @doi:10.1038/srep06662; @doi:10.1098/rsos.201424].
...
These two MIC estimators are available in the `minepy` package [@doi:10.1093/bioinformatics/bts707], a C implementation for Python. 

We compared all these coefficients in terms of computation time on randomly generated variables of different sizes, which simulate gene expression data with different numbers of conditions.
CCC allows us to easily parallelize the computation of a single gene pair (see [Methods](#sec:ccc_algo)), so we also tested the cases using 1 and 3 CPU cores.
...
We expected Pearson and Spearman to be the fastest, as they only compute basic summary statistics from the data.
In comparison, CCC was three orders of magnitude slower.
Among the nonlinear coefficients, CCC was faster than the two MIC variations, except for very small data sizes.
The difference is due to the implementation of MIC variants in C [@doi:10.1093/bioinformatics/bts707], a high-performance programming language, while CCC was implemented in Python (optimized with `numba`).
For data size of one million, the multi-core CCC was twice as fast as the single-core CCC.
This suggests that more advanced processing units (such as GPUs) could be used to speed up CCC, bringing it closer to the speed of Pearson.
...
## Table: Network Statistics of Six Gene Pairs

This table provides network statistics for six gene pairs shown in Figure @fig:upsetplot_coefs b for blood and predicted cell types.
The gene pairs are only those present in GIANT models.
For each gene in the pair (first column), the table includes the minimum, average, and maximum interaction coefficients with the other genes in the network.
{#tbl:giant:weights}
