...
We compared all the coefficients in this study with MIC [@pmid:22174245], a nonlinear method that can find complex relationships in data, but is computationally intensive [@doi:10.1098/rsos.201424].
...
This took 4 days and 19 hours, compared to 9 hours for CCC.
...
Figure @fig:dist_coefs_mic c shows that these two coefficients are almost linearly related, and both compare very similarly to Pearson and Spearman.
...
Although CCC and MIC might identify similar gene pairs in gene expression data, MIC is limited in large datasets due to its long computation time, despite some methodological and implementation improvements.
The original MIC implementation uses ApproxMaxMI, a computationally demanding heuristic estimator.
Recently, a more efficient implementation called MIC<sub>e</sub> was proposed.
These two MIC estimators are provided by the `minepy` package, a C implementation available for Python.
We compared all these coefficients in terms of computation time on randomly generated variables of different sizes, which simulates gene expression data with different numbers of conditions.
CCC allows for easy parallelization of the computation of a single gene pair.
Figure @fig:time_test shows the time in seconds in log scale using 1 and 3 CPU cores.
...
We expected Pearson and Spearman to be the fastest, as they only need to calculate basic statistics from the data.
For example, Pearson is three times faster than CCC.
Among the nonlinear coefficients, CCC was faster than the two MIC variations (up to two times faster) in most cases, except when the data size was very small.
This is because both MIC variants were written in C [@doi:10.1093/bioinformatics/bts707], a high-performance programming language, whereas CCC was written in Python (optimized with `numba`).
When the data size was one million, the multi-core CCC was twice as fast as the single-core CCC.
This suggests that new implementations using more powerful processing units (e.g.
GPUs) could make CCC reach speeds closer to Pearson.
