...
We compared the correlation coefficients in this study with MIC [@pmid:22174245], a popular nonlinear method that can detect complex relationships in data.
However, MIC is computationally intensive [@doi:10.1098/rsos.201424].
...
Figure @fig:dist_coefs_mic c indicates that these two coefficients are almost linearly related and both compare very similarly with Pearson and Spearman.
...
While CCC and MIC may identify similar gene pairs in gene expression data, MIC is not suitable for large datasets due to its lengthy computation time, despite various methodological and implementation improvements [@doi:10.1093/bioinformatics/bts707; @doi:10.1371/journal.pone.0157567; @doi:10.4137/EBO.S13121; @doi:10.1038/srep06662; @doi:10.1098/rsos.201424].
The original MIC computation uses ApproxMaxMI, a computationally intensive heuristic estimator [@doi:10.1126/science.1205438].
A more efficient version, called MIC<sub>e</sub>, has been proposed [@Reshef2016].
...
We tested the computation time of all coefficients on randomly generated variables of different sizes, which simulate gene expression data with various numbers of conditions.
CCC stands out as it allows for parallelization of the computation of a single gene pair (see [Methods](#sec:ccc_algo)).
We also tested the cases using 1 and 3 CPU cores.
...
Pearson and Spearman correlation coefficients proved to be the fastest, as expected, since they only require computing basic summary statistics from the data.
CCC was faster than the two MIC variations (up to two orders of magnitude), except for very small data sizes.
This is significant, since both MIC variants were programmed in C [@doi:10.1093/bioinformatics/bts707], a high-performance language, while CCC was implemented in Python with `numba` optimization.
With a data size of a million, the multi-core CCC was twice as fast as the single-core CCC.
This suggests that more advanced processing units (e.g.
GPUs) could be used to further improve the speed of CCC, bringing it closer to the speed of Pearson.
...
Table \ref{tbl:giant:weights} shows the network statistics of six gene pairs presented in Figure \ref{fig:upsetplot_coefs}b for blood and predicted cell types.
Only gene pairs that are present in GIANT models are listed.
For each gene in the pair (first column), the table displays the minimum, average, and maximum interaction coefficients with the other genes in the network.
