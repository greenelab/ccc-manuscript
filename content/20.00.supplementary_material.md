...
We compared the correlation coefficients from this study with MIC [@pmid:22174245], a popular nonlinear method for finding complex relationships in data.
However, MIC is very computationally intensive [@doi:10.1098/rsos.201424].
We ran MIC<sub>e</sub> (see Methods) on all possible pairwise comparisons of 5,000 highly variable genes from whole blood in GTEx v8.
This process took 4 days and 19 hours to finish, compared with 9 hours for CCC.
We analyzed the distribution of coefficients (as described in the main text) and the results are shown in Figure @fig:dist_coefs_mic.
We found that CCC and MIC have similar distributions, but shifted.
Figure @fig:dist_coefs_mic c shows that these two coefficients have a nearly linear relationship and they compare similarly to Pearson and Spearman.
...
CCC and MIC can identify similar gene pairs in gene expression data (see [here](#sec:mic)), but MIC is limited in larger datasets due to its long computation time, despite some methodological and implementation improvements [@doi:10.1093/bioinformatics/bts707; @doi:10.1371/journal.pone.0157567; @doi:10.4137/EBO.S13121; @doi:10.1038/srep06662; @doi:10.1098/rsos.201424].
...
A more efficient implementation called MIC<sub>e</sub> was recently proposed [@Reshef2016], which is provided by the `minepy` package [@doi:10.1093/bioinformatics/bts707], a C implementation available for Python.
We compared the computation time of these coefficients on randomly generated variables of different sizes, which simulates a gene expression data scenario with various numbers of conditions.
CCC is unique in that it allows for easy parallelization of the computation of a single gene pair (see [Methods](#sec:ccc_algo)), so we also tested the cases using 1 and 3 CPU cores.
...
Pearson and Spearman correlation coefficients are the fastest to compute, as they only require basic summary statistics from the data.
CCC is three orders of magnitude slower than Pearson, but is faster than the two MIC variations (up to two orders of magnitude) except in very small data sizes.
CCC was implemented in Python, while the MIC variants were implemented in C, a high-performance programming language.
For a data size of a million, the multi-core CCC was twice as fast as the single-core CCC.
This suggests that implementing CCC on more advanced processing units (such as GPUs) could increase its speed and bring it closer to that of Pearson.
