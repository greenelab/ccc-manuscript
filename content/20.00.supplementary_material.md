...
We compared our correlation coefficient with MIC [@pmid:22174245], a popular nonlinear method for finding complex relationships in data.
...
The distribution of coefficients (the same as in the main text) is shown in Figure @fig:dist_coefs_mic.
We verified that CCC and MIC have similar distributions, only shifted.
Figure @fig:dist_coefs_mic c shows that these two coefficients relate almost linearly, and both compare similarly to Pearson and Spearman.
...
CCC and MIC can identify similar gene pairs in gene expression data (see [here](#sec:mic)).
However, MIC's computation time is quite long, even with some methodological/implementation improvements [@doi:10.1093/bioinformatics/bts707; @doi:10.1371/journal.pone.0157567; @doi:10.4137/EBO.S13121; @doi:10.1038/srep06662; @doi:10.1098/rsos.201424].
...
This is provided by the `minepy` package [@doi:10.1093/bioinformatics/bts707], a C implementation available for Python.
We tested the computation time of all these coefficients on randomly generated variables of different sizes, which simulates a gene expression data scenario with various numbers of conditions.
Unlike the other coefficients, CCC allows us to easily parallelize the computation of a single gene pair (see [Methods](#sec:ccc_algo)).
We tested it using 1 and 3 CPU cores.
...
CCC was faster than the two MIC variations for most data sizes, except for very small sizes.
This is because the MIC variants were implemented in the high-performance programming language C [@doi:10.1093/bioinformatics/bts707], whereas CCC was implemented in Python (optimized with `numba`).
For a data size of a million, the multi-core CCC was twice as fast as the single-core CCC.
This indicates that more advanced processing units (such as GPUs) could be used to improve the speed of CCC, bringing it closer to the speed of Pearson and Spearman.
