...
We compared our correlation coefficient with MIC, a popular nonlinear method for finding complex relationships in data.
However, MIC is computationally intensive.
...
The analysis on the distribution of coefficients (the same as in the main text) is shown in Figure @fig:dist_coefs_mic.
Results showed that CCC and MIC behave similarly, with an almost linear relationship and similar performance to Pearson and Spearman.
...
CCC and MIC are able to identify similar gene pairs in gene expression data (see [here](#sec:mic)), however MIC's computation time is very long, even with some methodological/implementation improvements [@doi:10.1093/bioinformatics/bts707; @doi:10.1371/journal.pone.0157567; @doi:10.4137/EBO.S13121; @doi:10.1038/srep06662; @doi:10.1098/rsos.201424].
...
A more efficient implementation called MIC<sub>e</sub> was proposed [@Reshef2016], and is available in the `minepy` package [@doi:10.1093/bioinformatics/bts707].
We compared all these coefficients in terms of computation time on randomly generated variables of different sizes, simulating gene expression data with different numbers of conditions.
CCC is able to easily parallelize the computation of a single gene pair (see [Methods](#sec:ccc_algo)), so we also tested the cases using 1 and 3 CPU cores.
The results are shown in Figure @fig:time_test, in log scale.
...
Pearson and Spearman were found to be the fastest correlation coefficients, due to their reliance on basic summary statistics from the data.
CCC was faster than the two MIC variations, with the exception of very small data sizes.
This is likely because CCC was implemented in Python (with `numba` optimization), while the MIC variants were implemented in C, a high-performance programming language.
For datasets of a million, the multi-core CCC was twice as fast as the single-core CCC.
This indicates that more advanced processing units (such as GPUs) could be used to further improve CCC's speed, bringing it closer to the speed of Pearson and Spearman.
