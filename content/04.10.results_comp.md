...
These networks were built from 987 genome-scale data sets across approximately 38,000 conditions, including expression and different interaction measurements such as gene co-expression (using Pearson correlation after several preprocessing steps), protein-interaction, transcription factor regulation, and chemical and genetic perturbations and microRNA target profiles from the Molecular Signatures Database (MSigDB [@pmid:16199517]).
...
We also analyzed the top 10 gene pairs prioritized by CCC and not by either Pearson or Spearman, and by Pearson but not by CCC (no gene pairs had a high Spearman value and a low CCC).
- p_vs_c: top 10 that appear in GIANT
- c_vs_p: top 10 that appear in giant
- c_vs_s: top 10 that appear in giant excluding sex chromosomes



- the fact that pearson gene pairs are not replicated in GIANT suggests that this coefficient is very dependent on preprocessing of the input data (such as taking the logarithm or binning as done in GIANT)

- maybe add in supplementary the rest of the other gene pairs similar to rassf2 and cytip, which loook cool
  - and some are not in GIANT networks, so it would be interesting to leave them here
  - in the paragraph where we talk about kdm6a and uty, also cite ddx3y and kdm6a (it's the same pattern and ddx3y is from chr y)


- then, we left the GIANAT algorithm to select the best tissue
The tissue was automatically selected in each network according to the predicted tissue expression of gene pairs [CITE PAPER, and also mention something else].
[ADAPT LAST PART] Interestingly, gene pairs highly ranked by CCC are part of very cohesive networks and strongly related to blood.

[ADAPT BELOW, THESE ARE THE AUTOMATICALLY DETECTED CELL TYPES]
...
- analysis of top 10 genes by clustermatch v pearson, and pearson v clustermatch
  - measure connections in blood


- the analyses with GIANT tested two things: 1. preprocessing steps and 2. connection found in sub cell types of a tissue

<!-- ![
**Predicted blood-specific networks from GIANT for each gene pairs prioritized by correlation coefficients.**
A node represents a gene and an edge the probability that two genes are part of the same biological process in blood.
A maximum of 15 genes are shown for each subfigure.
...
*PYGM* - *TPM2* [@url:https://hb.flatironinstitute.org/gene/5837+7169].
The GIANT web-server was accessed on April 4, 2022.
](images/coefs_comp/giant_networks/blood_tissues/main.svg "GIANT network interaction"){#fig:giant_gene_pairs_blood width="100%"} -->
