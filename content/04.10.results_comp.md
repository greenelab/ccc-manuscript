...
We found that CCC behaves very similarly to MIC, although CCC performs two orders of magnitude faster than this coefficient (see [Supplementary Material](#sec:time_test)).
...
CCC, however, runs in a fraction of time, making it a practical alternative on large data sets.


Next, we selected individual gene pairs where CCC disagreed with the rest.
We select gene pairs among the top five of each intersection in the "Disagreements" group (Figure @fig:upsetplot_coefs b, right) where CCC disagrees with either Pearson, Spearman or both.
...
In studies of the transcriptome, a not-only-linear correlation coefficient like CCC could identify sex-specific differences in gene expression that, like the *UTY* - *KDM6A* example, are completely missed by linear-only coefficients.
...
The evidence of interaction between *ZDHHC12* and *CCL18* was less evident:
...
*MYOZ1* and *TNNI2* had both low average interaction scores of 0.17 and 0.22 with the rest of the genes.
...
For this, we let GIANT to automatically predict a relevant tissue- or cell type-specific network (see [Methods](#sec:giant)).
...
The average probability of interaction between genes in these CCC-ranked networks was also very high, with all medians close to 75% and 1st quartiles above 40% in all predicted tissues/cell types.
...
The gene-gene relationships exclusively detected by CCC in whole blood from GTEx were replicated in more sophisticated models trained without GTEx.
These results suggest that CCC is able to capture cell lineage-specific transcriptional patterns that are missed by linear-only coefficients.
