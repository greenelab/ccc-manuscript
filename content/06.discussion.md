...
Applying CCC to GTEx v8 revealed that it was robust to outliers and detected linear relationships as well as complex and biologically meaningful relationships that standard coefficients missed.
CCC alone detected gene pairs from the sex chromosomes following complex nonlinear patterns.
These examples, in particular, highlight the way that not-only-linear coefficients can play in capturing sex-specific differences.
...
The CCC results were concordant with MIC on the random subset that we were able to analyze, but much faster to compute and thus practical for large datasets datasets.
CCC is conceptually easy to interpret and has a single parameter that controls the complexity of the detected relationships while also balancing compute time.
...
Some genes highlighted in pairs prioritized by CCC (Figure @fig:upsetplot_coefs b), such as *SDS* (12q24) and *ZDHHC12* (9q34), were previously found to be the focus of fewer than expected publications [@pmid:30226837]. <!-- did i interpret this correctly? -->
...
Our results reveal that the advantages of using more advanced coefficients like CCC for detecting and studying more intricate molecular mechanisms.
...
We used $k_{\mathrm{max}}=10$ in our analyses which our analysis suggests was sufficient to identify both linear and more complex patterns in gene expression.
...
While it is possible to explicitly design analyses to identify sex-specific relationships, being able to use a method that inherently identifies them is likely to be desirable.
