...
Finally, since correlation coefficient does not have a lower bound (it could return negative values, which in our case are not meaningful), CCC returns only values between 0 and 1 (line 17).
...
We found that the not-only-linear correlation coefficient (NCC) is a better metric for comparing gene expression levels between samples than the Pearson correlation coefficient (PCC).
The NCC is more efficient because it does not require the calculation of the covariance matrix.
We used three CPU cores in our analyses to speed up the computation of NCC.
A future improved implementation of CCC could potentially use graphical processing units (GPU) to parallelize its computation further.
