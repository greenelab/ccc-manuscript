...
\begin{algorithm}[t]
  ...
  \Fn{\FPartitions{$\vect{v}$, $k_{\mathrm{max}}$}}{
      ...
      \If{$\vect{v} \in \mathbb{R}^n$}{\label{alg:num}
        \For{$k \leftarrow 2$ \KwTo $\min\lbrace k_{\mathrm{max}},\left\vert\vect{v}\right\vert - 1 \rbrace$}{
      ...
      \Else{\label{alg:cat}
        $\mathcal{C} \leftarrow \{v_{i} | v_{i} \in \mathbf{v}\}$\;\label{alg:ref4}
        $k \leftarrow \left\vert\mathcal{C}\right\vert$\;\label{alg:ref5}
        $\pi_{c} \leftarrow \{i \mid v_{i} = \mathcal{C}_{c}\}, \forall c \in \left[1,k\right]$\;
        $\Pi_{k} \leftarrow \pi$\;
      }
