...
Gene expression data were log-transformed, and the correlation was computed for each gene pair, normalized using the Fisher's z transform, and z-scores discretized into different bins.
...
We computed the correlation coefficient between the gene expression values in each tissue and the gene expression values in the blood network.
We also computed the correlation coefficient between the gene expression values in each tissue and the gene expression values in the predicted tissue network.
Finally, we computed the correlation coefficient between the gene expression values in each tissue and the gene expression values in the whole blood network.
